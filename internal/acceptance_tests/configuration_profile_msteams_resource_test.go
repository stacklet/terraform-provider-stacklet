...
func TestAccConfigurationProfileMSTeamsResource_RequiresReplaceIfCustomerConfigUnset(t *testing.T) {
	steps := []resource.TestStep{
		// Create initial resource with both access_config_input and customer_config_input
		{
			Config: `
		...
		// Try to remove customer_config_input - should require replacement
		{
			Config: `
				resource "stacklet_configuration_profile_msteams" "test" {
					access_config_input = {
	...
	runRecordedAccTest(t, "TestAccConfigurationProfileMSTeamsResource_RequiresReplaceIfCustomerConfigUnset", steps)
}
