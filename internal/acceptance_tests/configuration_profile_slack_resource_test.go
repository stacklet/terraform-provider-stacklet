...
func TestAccConfigurationProfileSlackResource(t *testing.T) {
	steps := []resource.TestStep{
		// Create and Read testing - webhooks intentionally not in alphabetical order
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttrSet("stacklet_configuration_profile_slack.test", "webhook.0.url"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.1.name", "bar"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.1.url_wo_version", "1"),
				resource.TestCheckResourceAttrSet("stacklet_configuration_profile_slack.test", "webhook.1.url"),
			),
		},
		// ImportState testing
		{
			ResourceName:      "stacklet_configuration_profile_slack.test",
			ImportState:       true,
			...
			ImportStateVerifyIgnore: []string{
				...
				"token_wo", "token_wo_version", "token",
			},
		},
		// Update and Read testing
		{
			Config: `
				resource "stacklet_configuration_profile_slack" "test" {
					...
					webhook {
						name = "bar-new"
						url_wo = "https://example.com/webhooks/two-new"
						url_wo_version = "1"
					}
				}
			`,
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttrSet("stacklet_configuration_profile_slack.test", "profile"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.#", "2"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.0.name", "foo-new"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.0.url_wo_version", "1"),
				resource.TestCheckResourceAttrSet("stacklet_configuration_profile_slack.test", "webhook.0.url"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.1.name", "bar-new"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.1.url_wo_version", "1"),
				resource.TestCheckResourceAttrSet("stacklet_configuration_profile_slack.test", "webhook.1.url"),
			),
		},
	}
	runRecordedAccTest(t, "TestAccConfigurationProfileSlackResource", steps)
}

func TestAccConfigurationProfileSlackResource_URLChange(t *testing.T) {
	...
	steps := []resource.TestStep{
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.1.url_wo_version", "1"),
				resource.TestCheckResourceAttrSet("stacklet_configuration_profile_slack.test", "webhook.1.url"),
				resource.TestCheckResourceAttrWith("stacklet_configuration_profile_slack.test", "webhook.0.url", func(value string) error {
					returnedURL1 = value
					return nil
				}),
				resource.TestCheckResourceAttrWith("stacklet_configuration_profile_slack.test", "webhook.1.url", func(value string) error {
					returnedURL2 = value
					return nil
				}),
			),
		...
		// Version changed for first webhook, URL should change
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.0.name", "foo"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.0.url_wo_version", "2"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.1.name", "bar"),
				resource.TestCheckResourceAttr("stacklet_configuration_profile_slack.test", "webhook.1.url_wo_version", "1"),
				resource.TestCheckResourceAttrWith("stacklet_configuration_profile_slack.test", "webhook.0.url", func(value string) error {
					if value == returnedURL1 {
	...
	runRecordedAccTest(t, "TestAccConfigurationProfileSlackResource_URLChange", steps)
}
